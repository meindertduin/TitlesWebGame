...
namespace TitlesWebGame.Api.Services
{
    public class CompetitiveArtistRoundController : IGameRoundController
    {
        ...
        public async Task PlayGameRound(GameSessionState gameSessionState, GameRoundInfo gameRoundInfo)
        {
            ...
            await _clientMessageService.UpdatePlayersOfNewRoundInfo(gameSessionState.RoomKey, new CompetitiveArtistUploadRoundInfoViewModel()
            {
                RoundTimeMs = 3000,
                GameRoundsType = GameRoundsType.CompetitiveArtistUploadRound,
                RewardPoints = 0,
                RoundStatement = String.Empty,
            });
